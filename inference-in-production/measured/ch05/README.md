...
The ch05 calculator as recorded on the reference laptop (Apple M2 Pro,
16 GB, OrbStack 29.4.0, linux/arm64) on 2026-09-19:

```sh
AIEL_MACHINE="Apple M2 Pro, 16 GB, OrbStack 29.4.0 (linux/arm64 VM)" \
  docker compose run --rm inference-in-production ch05
```
...
- `derived.json`: every DERIVED number the chapter prints, with its
  ...
  difference.
- `other-shape.txt`: the calculator on a second shape (20,000 in /
  500 out at 41.2% utilization), which it warns it cannot price.
