...
entry point is `docker compose run --rm inference-engineering-in-practice
...
`docker compose run --rm -T inference-engineering-in-practice test chNN` on
...
- `machine.json` from `write_manifest`: the date, the Docker host,
  ...
  VM)" docker compose run --rm inference-engineering-in-practice chNN`;
...
- A lab that crashed outside the trap (a killed Docker daemon) can
  ...
  inference-engineering-in-practice clean` removes every container labelled
  `aiel.lab`.
- After changing `Dockerfile` or `requirements.txt`, rebuild the
  runner: `docker compose build inference-engineering-in-practice`.
