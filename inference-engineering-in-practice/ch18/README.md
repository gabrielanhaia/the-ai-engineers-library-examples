...
1. **Quality gate** (`gate.py quality`). Runs chapter 11's task eval
   (`../ch11/eval.py`, 40 questions, greedy) against llama.cpp
   serving SmolLM2-360M-Instruct Q8_0 three times: the baseline
   with the model's own chat template, a rerun of the same, and a
   candidate whose template ignores `add_generation_prompt`
   (`broken_template.py`). The gate fails a candidate that loses
   more than 2 correct answers or changes more than 10% of the
   baseline's replies: at temperature 0 on the same weights, a
...
2. **Performance gate** (`gate.py performance`). Runs
   ...
   Goodput counts requests inside the SLO (TTFT under 1 s, TPOT
   under 25 ms); the gate fails a candidate below 90% of the
   baseline's goodput. **Simulated**: the simulator's timings are
   its configuration, so this proves the gate's logic, not a GPU's
   speed.
...
and the models `smollm2-360m-gguf` and `hf`, which it downloads
once. It took 9 minutes on the reference laptop.
...
performance gate, candidate: rerun
  ok   goodput: 1.81 req/s, baseline 1.80 (100%, need 90%)
  ok   median TPOT: 21.9 ms, baseline 21.9 ms
  performance gate: PASS
performance gate, candidate: slower-config
  FAIL goodput: 0.00 req/s, baseline 1.80 (0%, need 90%)
  ok   median TPOT: 27.1 ms, baseline 21.9 ms
...
fails the broken template; the performance gate passes the rerun
...
- `gate.py`: both gates.
- `broken_template.py`: the model's chat template, and the copy
  with its generation prompt deleted.
...
- Output lands in `measured/ch18/`: `gates.json`, the three eval
  results (`eval-*.json`) and the three benchmark results
  (`bench-*.json`).
